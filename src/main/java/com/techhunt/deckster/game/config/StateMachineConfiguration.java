...
import static com.techhunt.deckster.game.enums.GameEvent.DEALT;
import static com.techhunt.deckster.game.enums.GameEvent.GAME_SET;
import static com.techhunt.deckster.game.enums.GameEvent.PROMPTED;
import static com.techhunt.deckster.game.enums.GameEvent.RESPONDED;
import static com.techhunt.deckster.game.enums.GameEvent.REWARDED;
...
public class StateMachineConfiguration extends StateMachineConfigurerAdapter<GameState, GameEvent> {
    ...
    public void configure(StateMachineTransitionConfigurer<GameState, GameEvent> configurer) throws Exception {
        configurer.withExternal()
                ...
                .withExternal()
                    .source(SETUP).target(DEAL).event(GAME_SET).and()
                .withExternal()
                    .source(DEAL).target(PROMPT).event(DEALT).and()
                .withExternal()
                    .source(PROMPT).target(RESPONSES).event(PROMPTED).and()
                .withExternal()
                    .source(RESPONSES).target(REWARD).event(RESPONDED).and()
                .withExternal()
                    .source(REWARD).target(WINNER).event(REWARDED).and()
                .withChoice()
